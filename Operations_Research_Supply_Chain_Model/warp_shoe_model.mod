...
subject to RM_Budget: sum{i in Product_Num, p in RM_Num} Cost[p]*Quantity[i,p]*x[i] <= 10000000;         # constraint for the budget of raw materials
...
subject to Demand_Limit{i in Product_Num}: x[i] <= 2*round(Product_Demand[i]);                           # constraint that limits the amount of shoe i produced based on the monthly demand
